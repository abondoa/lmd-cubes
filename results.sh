...
for SCALE in $SCALES; do
    ...
    RESULT_FILE="results"$LOGGED_SCALE".csv"
    rm $RESULT_FILE
    ...
    for CUBE in $CUBES ; do
        for QUERY in $(ls $HOME../../queries/TPCH/snowflakeschema/$CUBE"_"cube/ | egrep -o '[0-9]+' | sort -n); do
            echo "Query "$QUERY
            echo -n "{$CUBE},$QUERY" >> $RESULT_FILE
            for OPTIMIZATION in  $OPTIMIZATIONS ; do
                ...
                for TIMEOUTQUERY in $TIMEOUTQUERIES; do
                    if [[ $(echo $TIMEOUTQUERY | egrep "$OPTIMIZATION/$CUBE"_"cube/query$QUERY.txt") != "" ]]; then
                ...
                for SEED in $SEEDS; do
                    ...
                    for RUN in `seq 0 $(echo "$RES_PER_FILE - 1" | bc -l)`; do
                        ...
                        if [[ $TIMEOUT = true ]]; then
                            ...
                            grep -Pzo "(?s)Query $QUERY of run $RUN has been executed in [0-9\.]*" $BIBM/$EXTBI_RELATIVE_TO_BIBM/data/TPCH/$THISHOST/$REPO"_"$SEED.tmp | perl -pe "s/Query $QUERY of run $RUN has been executed in ([0-9\.]*)/\$1/" >> tmp
                        fi
                    done
                done
